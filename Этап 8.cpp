...
public:
    Meeting(const std::string& meetingTopic, const std::vector<Teacher>& meetingTeachers, const std::vector<Parent>& meetingParents)
        : topic(meetingTopic), teachers(meetingTeachers), parents(meetingParents) {}
    ...
    void parentsSpeakAboutChildren() const {
        std::cout << "Высказывания родителей о своих детях:" << std::endl;
...
int main() {
    ...
    Meeting schoolMeeting("Обсуждение занятий", teachersForMeeting, parentsForMeeting);

    schoolMeeting.discussLessons(school);
    schoolMeeting.parentsSpeakAboutChildren();
